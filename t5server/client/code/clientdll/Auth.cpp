...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	if (!doneFile && (err = fopen_s(&iw4mini, "t5m.ini", "r")) == 0){
	...
	info.pszCaptionText = L"Authentication for RepZOps";
	info.pszMessageText = L"Please log in to use RepZOps. Use your RepZIW4.de forum credentials.";
	...
	if (result == ERROR_SUCCESS)
	{
		...
		*passwordP = password;
		...
		if (save && (err = fopen_s(&iw4mini, "t5m.ini", "w")) == 0){
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://%s/remauth.php", "null");
		...
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "repzOps");
...
void GetCountryCode()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://dw.repziw4.de/getloc.php");
